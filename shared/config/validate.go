...
func (v *Validator) CheckRow(sa []string, filename, k string, line int) error {
	if len(sa) == 0 {
		// 如果有默认值，跳过
		if !v.allowEmpty && len(v.defaults) <= 0 {
		...
		if len(s) > 0 {
			...
			if v.regexp != nil {
				if !v.regexp.MatchString(s) {
					return errors.Errorf("校验配置文件[%s](行数: %v)，字段[%s]必须是%s [pattern: %s], %s %s", filename, line, k, v.tips, v.regexpPattern, sa, s)
				}
			}
...
func ParseValidator(str, sep string, array bool, wlist []string, defaults []string, allowEmpty bool) *Validator {
	...
	if out != nil {
		if len(parts) == 1 && len(wlist) == 0 && len(defaults) == 0 && !allowEmpty {
			return out
		}
		...
		if vv := validatorMap[parts[0]]; vv != nil {
			t := *vv
			out = &t
		} else {
...
var (
	...
	float64Validator        = newRegexpValidator(`^[-+]?(\d+(\.\d*)?|\.\d+)([eE]([-+]?([012]?\d{1,2}|30[0-7])|-3([01]?[4-9]|[012]?[0-3])))?$`, "数字（支持小数）")
	float64Le0Validator     = newRegexpValidator(`^(\d+(\.\d*)?|\.\d+)([eE]([-+]?([012]?\d{1,2}|30[0-7])|-3([01]?[4-9]|[012]?[0-3])))?$`, "正数（支持小数）>=0")
